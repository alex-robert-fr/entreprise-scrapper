...
const MOBILE_SQL = sql`(
  telephone LIKE '06%' OR telephone LIKE '07%' OR
  telephone LIKE '+336%' OR telephone LIKE '+337%' OR
  telephone LIKE '336%' OR telephone LIKE '337%'
)`;

export function phoneTypeCondition(phoneType: "mobile" | "fixe"): SQL {
  if (phoneType === "mobile") return MOBILE_SQL;
  return sql`(telephone IS NOT NULL AND telephone != '' AND NOT ${MOBILE_SQL})`;
}
